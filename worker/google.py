...
def google_login(page, acc: dict) -> None:
    ...
    while time.time() < deadline:
        ...
        if step == "consent":
            log("检测到授权页，点击同意授权")
            click_one_of(page, CONSENT, 25000, "同意授权")
            log("等待离开授权页")
            try:
                wait_leave(page, "/signin/oauth", 45000, "授权页")
            except Exception as exc:
                log("%s", exc)
            continue
        ...
        if step == "password" or (email_done and visible(page, PASS_SEL)):
            if not visible(page, PASS_SEL):
                log("已到密码页，等待密码框出现")
                try:
                    wait_visible(page, PASS_SEL, 20000)
                except LoggedIn:
                    return
                except Exception:
                    sleep_ms(500)
            ...
            log("等待离开密码页")
            try:
                wait_leave(page, "/challenge/pwd", 30000)
            except Exception:
                pass
            if logged_in(page):
                return
        ...
        if not email_done and (step == "email" or (not step and visible(page, EMAIL_SEL) and not visible(page, PASS_SEL))):
            ...
            log("等待跳转到密码页")
            try:
                wait_path_contains(page, "/challenge/pwd", 30000)
            except Exception:
