...
def launch_ctx(settings: dict, seed: int):
    ...
    args = ["--disable-setuid-sandbox", "--disable-dev-shm-usage", "--fingerprint-storage-quota=5000", "--ignore-gpu-blocklist"]
    if seed > 0:
        args.append(f"--fingerprint={seed}")
    log("Cloak 官方包装启动 humanize=True geoip=%s headless=%s seed=%s", bool(proxy), headless, seed)
    kwargs = {
